...
Una vez creada la estructura de datos, podemos acceder a la información de las muestras con la función
...
Y a la información de los genes con la función
...
También podemos mostrar el diseño experimental con la función
...
Mostramos la información de las muestras del ejemplo.
...
- `results(dds, contrast = c(factor, grupo1, grupo2))`: Muestra una tabla con los principales estadísticos del contraste de comparación de expresión génica del `grupo2` frente al `grupo1` del factor experimental dado en `factor`, realizado do a partir de la estructura de datos en formato `DESeqDataSet` dada en `dds`. 
...
Siguiendo con nuestro ejemplo, primero definirmos el grupo Basal como la primera categoría del factor `group`.

```{r}
dds$group <- relevel( dds$group, "Basal" )
```
...
```{r}
res_B_LP <- results(dds, contrast = c("group", "Basal", "LP"))
res_B_LP
summary(res_B_LP)
```
...
```{r}
res_B_ML <- results(dds, contrast = c("group", "Basal", "ML"))
res_B_ML
summary(res_B_ML)
```
...
```{r}
res_LP_ML <- results(dds, contrast = c("group", "LP", "ML"))
res_LP_ML
summary(res_LP_ML)
```
:::
